...
summer_item[int] submit_items(summer_item[int] summer_items) {
   string post_url_items = "http://www.plasticlobster.com/summer/summer_items.php?class="+url_encode(my_class())+"&sign="+url_encode(my_sign());
...
summer_consumable[int] submit_consumables(summer_consumable[int] summer_consumables) {
   string post_url_consumables = "http://www.plasticlobster.com/summer/summer_consumables.php?class="+url_encode(my_class())+"&sign="+url_encode(my_sign());
...
void main() {
   ...
   foreach b in fake_item_list {
      ...
      if (!(known_items[fake_item_list[b]] > 0)) {
         fake_summer_consumables[count(fake_summer_consumables)] = spade_fake_consumable(fake_item_list[b], b);
         j++;
      }
      if (j % 25 == 0) {
         print("Spading Item "+fake_item_list[b]);
      }
